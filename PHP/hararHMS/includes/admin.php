...
function adduser()
{
	...
	if (mysqli_num_rows($query1)==0) {
		$sql = "INSERT INTO `users` VALUES ('$username','$password','$fname','$sname','$type')";
		$query = mysqli_query($con,$sql);
		if (!empty($query)) {
...
function updateuser()
{
	...
	$name = $_GET['name'];
		...
		$sql = "UPDATE `users` SET `fname`='$fname',`sname`='$sname',`type`='$type',`password`='$password' WHERE `username`='$name'";
		$query =mysqli_query($con,$sql);
		if (!empty($query)) {
...
function settings()
{
	...
	if ($password != $password) {
		...
		$type = $_SESSION['type'];
				...
				$sql = "UPDATE `users` SET `fname`='$fname',`sname`='$sname',`password`='$password' WHERE `username`='$name' AND `type`='$type'";
				$query = mysqli_query($con,$sql);
				if (!empty($query)) {
